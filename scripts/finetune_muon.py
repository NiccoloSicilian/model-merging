...
from model_merging.model.muon_image_classifier import MuonImageClassifier
...
def run(cfg: DictConfig):
    ...
    # Unfreeze encoder so Muon can update it
    ...
    model: MuonImageClassifier = hydra.utils.instantiate(
        cfg.nn.module,
    ...
    # Save encoder with naming: muon_ft_{dataset}_{epochs}ep.pt
    epochs = cfg.train.trainer.max_epochs
    save_name = f"muon_ft_{cfg.dataset.name}_{epochs}ep.pt"
    save_path = os.path.join(PROJECT_ROOT, save_name)
    torch.save(model.encoder.state_dict(), save_path)
    pylogger.info(f"Encoder saved to {save_path}")
...
@hydra.main(config_path=str(PROJECT_ROOT / "conf"), config_name="finetune_muon.yaml")
