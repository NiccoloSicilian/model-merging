...
class DualMerger(TaskVectorBasedMerger):
    ...
    def __init__(
        ...
        self.dualize_pretrained = dualize_pretrained
    ...
    def merge(self, base_model, finetuned_models):
        ...
        svd_dict = get_svd_dict(task_dicts, datasets, self.svd_path, self.svd_compress_factor)
        ...
        module_net = build_duality_map(
            ordered_keys, multi_task_vector_cpu, self.device, self.mass_schedule, self.model_name
        )  # ← add self.device
        ...
        multi_task_vector_cpu = {k: v.to(self.device) for k, v in multi_task_vector_cpu.items()}
        ...
        if self.dualize_pretrained:
            base_state_dict_cpu = {
                k: v.detach().cpu() for k, v in merged_encoder.state_dict().items()
            }
            base_ordered_keys = get_vit_topological_order(list(base_state_dict_cpu.keys()))
            base_dual_map = build_duality_map(
                base_ordered_keys,
                base_state_dict_cpu,
                self.device,
                self.mass_schedule,
            ...
            for key in base_dual_map:
                base_state_dict_cpu[key] = base_dual_map[key].cpu()
            merged_encoder.load_state_dict(base_state_dict_cpu, strict=False)
            del base_state_dict_cpu
            del base_dual_map
            gc.collect()
